...
static long long keygen_falcon_cycle, eval_falcon_keygen_cycle, eval_falcon_sign_cycle, verify_falcon_cycle;
...
void keygen(TREE_NODE *tree, AES256_CTR_DRBG_struct *s, AES256_CTR_DRBG_struct *s_prime)
{
	...
	for (i = 0; i < N; i++)
	{
		...
		falcon_keygen_make(&sc_i, FALCON_LOGN, sk_i, FALCON_PRIVKEY_SIZE(FALCON_LOGN), pk_i, FALCON_PUBKEY_SIZE(FALCON_LOGN), buf, FALCON_TMPSIZE_KEYGEN(FALCON_LOGN));
		...
		memcpy(buf, tree[N + i].hash, HASH_LENGTH);
		memcpy(buf + HASH_LENGTH, pk_i, FALCON_PUBKEY_SIZE(FALCON_LOGN));
		SHA256(buf, HASH_LENGTH + FALCON_PUBKEY_SIZE(FALCON_LOGN), tree[N + i].hash);
	}	
...
void eval(unsigned char *v, unsigned char *y, TREE_NODE *ap, unsigned char *pk, unsigned char *sig, size_t *sig_len, const unsigned char *mu1, const unsigned char *mu2, const uint32_t i_in, const uint32_t j_in, const AES256_CTR_DRBG_struct *s, const AES256_CTR_DRBG_struct *s_prime, const TREE_NODE *tree)
{
	...
	falcon_keygen_make(&sc_key, FALCON_LOGN, sk, FALCON_PRIVKEY_SIZE(FALCON_LOGN), pk, FALCON_PUBKEY_SIZE(FALCON_LOGN), buf, FALCON_TMPSIZE_KEYGEN(FALCON_LOGN));
	
	cycle2 = cpucycles();
	/* sig <-- Falcon.Sign(sk, \mu_2) */
	shake256_init_prng_from_system(&sc_sig);
	*sig_len = FALCON_SIG_COMPRESSED_MAXSIZE(FALCON_LOGN);
	falcon_sign_dyn(&sc_sig, sig, sig_len, FALCON_SIG_COMPRESSED, sk, FALCON_PRIVKEY_SIZE(FALCON_LOGN), mu2, MU_LENGTH, buf, FALCON_TMPSIZE_SIGNDYN(FALCON_LOGN));
...
uint32_t verify(const unsigned char *mu1, const unsigned char *mu2, const uint32_t i_in, const uint32_t j_in, const unsigned char *v, const unsigned char *y, const TREE_NODE *ap, const unsigned char *pk, const unsigned char *sig, const size_t sig_len, const TREE_NODE *root)
{
	...
	falcon_verify_res = falcon_verify(sig, sig_len, FALCON_SIG_COMPRESSED, pk, FALCON_PUBKEY_SIZE(FALCON_LOGN), mu2, MU_LENGTH, buf, FALCON_TMPSIZE_VERIFY(FALCON_LOGN));
...
int main()
{
	...
	for (benchmark_iteration = 0; benchmark_iteration < BENCHMARK_ITERATION; benchmark_iteration++)
	{
		...
		cycle4 = cpucycles();
		verify_res = verify(mu1, mu2, i_in, j_in, v, y, ap, pk, sig, sig_len, tree + 1);
		cycle5 = cpucycles();
		
		printf("%lld,%lld,%lld,%lld,%lld,%u\n", cycle4 - cycle3, eval_falcon_keygen_cycle, eval_falcon_sign_cycle, cycle5 - cycle4, verify_falcon_cycle, verify_res);
	}
